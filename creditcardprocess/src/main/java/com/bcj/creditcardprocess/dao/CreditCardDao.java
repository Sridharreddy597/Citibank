...
public class CreditCardDao {
	...
	public List<Customer> getCustomerList() {
		...
		// custs = session.createQuery("FROM Customer WHERE
		// status='New'").list();

		Session session = this.sessionFactory.getCurrentSession();
		Transaction transaction =  session.beginTransaction();
		customerList = (ArrayList<Customer>) session.createQuery("FROM Customer WHERE status='New'").list();
		// transaction.commit();
		// session.close();
		// return customerList;
	...
	public void persistCreditCard(CreditCard cCard, Customer cust) {
		...
		// FileSystemXmlApplicationContext("resources\\spring.xml");
		// SessionFactory obj = (SessionFactory)
